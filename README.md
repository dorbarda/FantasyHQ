...
`YOUTUBE_API_KEY` (or, as a fallback, `HIGHLIGHTLY_API_KEY`) is an optional extra secret for recap highlight clips.
...
| `YOUTUBE_API_KEY` | Optional. Highlight videos on the weekly recap: the week's best fantasy games on our rosters, with the player's clip (or his game's) from the official NBA YouTube channel. Free Google Cloud key with the YouTube Data API v3 enabled. Unset = no highlights, nothing else changes |
| `HIGHLIGHTLY_API_KEY` | Optional fallback for highlights when `YOUTUBE_API_KEY` is unset. Its free plan returns no NBA clips (checked 2026-09) |
