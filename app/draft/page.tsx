...
export default async function DraftPage({ searchParams }: PageProps) {
  if (!hasEspnCredentials()) {
    return (
      <div className="py-5">
        <h1 className="text-[28px] font-bold tracking-tight text-[#0f1419] mb-2">Draft Board</h1>
  ...
  return (
    <div className="py-5">
      {/* Header */}
      <div className="mb-4">
        <h1 className="text-[28px] font-bold tracking-tight text-[#0f1419]">Draft Board</h1>
      ...
      {error && (
        <p className="text-[15px] text-[#536471]">Could not load draft data — try again later.</p>
      )}

      {data && data.picks.length === 0 && (
        <div className="border border-[#eff3f4] rounded-2xl px-6 py-10 text-center">
          <p className="text-[15px] font-bold text-[#0f1419]">Draft not yet held</p>
          <p className="text-[13px] text-[#536471] mt-1">Check back once the {data.seasonLabel} draft is complete.</p>
        </div>
      )}
      ...
      {data && data.picks.length > 0 && (
        <>
          {/* Summary row */}
          <div className="flex items-center gap-4 mb-4 text-[13px] text-[#536471]">
            <span>{data.rounds} rounds · {data.picks.length} picks · {data.teams.length} teams</span>
            {!data.hasStats && (
              <span className="text-[#ff7a00] font-medium">Season in progress — grades pending</span>
            )}
          </div>
