...
function StatPill({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="flex flex-col items-center bg-white/10 backdrop-blur-sm rounded-xl px-4 py-3 min-w-[80px]">
      <span className={`text-[20px] font-black leading-tight ${accent ? 'text-warning' : 'text-white'}`}>
        {value}
      </span>
...
function H2HRow({
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-2.5 ${rowBg} border-b border-border last:border-0`}>
      <div className="w-28 shrink-0">
        <p className="text-[13px] font-semibold text-foreground truncate">{opponent}</p>
      </div>
...
function ResultBadge({ isChampion, isRunnerUp, isThird, isLast }: {
  ...
  if (isChampion) return <span className="text-[11px] font-bold bg-warning-surface text-warning-text border border-warning-bright rounded-full px-2 py-0.5">🏆 Champion</span>;
  if (isRunnerUp) return <span className="text-[11px] font-bold bg-surface-secondary text-secondary border border-border-strong rounded-full px-2 py-0.5">🥈 Runner-up</span>;
  if (isThird)    return <span className="text-[11px] font-bold bg-bronze-surface text-warning-text border border-bronze rounded-full px-2 py-0.5">🥉 3rd</span>;
  if (isLast)     return <span className="text-[11px] font-bold bg-negative-surface text-negative border border-negative-bright rounded-full px-2 py-0.5">🚽 Last</span>;
  return null;
}
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="px-4 sm:px-6 lg:px-8 pt-5">
        <Link
          href="/teams"
          className="inline-flex items-center gap-1.5 text-[12px] font-medium text-tertiary hover:text-accent transition-colors"
        >
      ...
      {/* ── HERO BANNER ────────────────────────────────────────────────────────── */}
      <div className="mx-4 sm:mx-6 lg:mx-8 mt-3 mb-6 rounded-2xl overflow-hidden bg-gradient-to-br from-panel via-panel to-panel-secondary shadow-xl">
        ...
        {champYears.length > 0 && (
          <div className="border-t border-white/10 px-6 py-3 flex items-center gap-2 flex-wrap">
            ...
            {champYears.map(y => (
              <span key={y} className="text-[11px] font-bold bg-warning/20 text-warning-bright border border-warning/30 rounded-full px-2.5 py-0.5">
                {y}
              </span>
            ))}
            {lastPlaceCount > 0 && (
              <>
                ...
                <span className="text-[11px] font-bold bg-negative-bright/20 text-negative-bright border border-negative-bright/30 rounded-full px-2.5 py-0.5">
                  {lastPlaceCount}×
                </span>
              </>
            )}
      ...
      <div className="px-4 sm:px-6 lg:px-8 space-y-6 pb-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          ...
          <div className="lg:col-span-3 bg-surface border border-border rounded-2xl p-5">
            ...
            <div className="flex items-center gap-3 mt-3 flex-wrap">
              {[
                { color: 'var(--warning)', label: '1st' },
                { color: 'var(--foreground-muted)', label: '2nd' },
                { color: 'var(--accent)', label: '3rd' },
                { color: 'var(--info-bright)', label: 'Other' },
                { color: 'var(--negative-bright)', label: 'Last' },
              ].map(({ color, label }) => (
            ...
            <div className="divide-y divide-border">
              {[...ownerSeasons].reverse().map(s => (
                <div key={s.year} className="px-4 py-3 flex items-start gap-3">
                  <div className="text-[11px] font-mono text-muted shrink-0 mt-0.5 w-12">
                    {s.seasonLabel}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[12px] font-semibold text-foreground truncate">{s.teamName}</p>
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      <span className="text-[11px] text-secondary font-mono">{s.wins}–{s.losses}</span>
                      <span className="text-[10px] text-muted">{s.pf.toLocaleString()} PF</span>
                      <ResultBadge
                        isChampion={s.isChampion}
                        isRunnerUp={s.isRunnerUp}
                        isThird={s.isThird}
                  ...
                  <span className="text-[11px] font-bold text-tertiary shrink-0">#{s.rank}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-surface border border-border rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between">
            ...
            <span className="text-[11px] text-tertiary">{h2hEntries.length} opponents</span>
          </div>
          ...
          {(bestOpponent || worstOpponent) && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-border border-b border-border">
              {bestOpponent && (
                <div className="bg-positive-surface px-5 py-4">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-positive mb-1">
                    👑 Favourite Prey
                  </p>
                  <p className="text-[16px] font-black text-positive">{bestOpponent.opponent}</p>
                  <p className="text-[13px] font-mono text-positive mt-0.5">
              ...
              {worstOpponent && (
                <div className="bg-negative-surface px-5 py-4">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-negative-bright mb-1">
                    😬 Kryptonite
                  </p>
                  <p className="text-[16px] font-black text-negative">{worstOpponent.opponent}</p>
                  <p className="text-[13px] font-mono text-negative-bright mt-0.5">
          ...
          {/* Full H2H table */}
          <div>
            <div className="grid grid-cols-[7rem_1fr_4rem_3rem_1.5rem] gap-x-3 px-4 py-2 bg-background border-b border-border">
              <span className="text-[10px] font-semibold uppercase tracking-wider text-muted">Opponent</span>
              <span className="text-[10px] font-semibold uppercase tracking-wider text-muted">Win Rate</span>
              <span className="text-[10px] font-semibold uppercase tracking-wider text-muted text-right">W–L</span>
              <span className="text-[10px] font-semibold uppercase tracking-wider text-muted text-right">%</span>
              <span />
            </div>
            {h2hEntries.map(e => (
              <H2HRow
                ...
                <div key={i} className="bg-surface px-5 py-4">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[18px] font-black text-foreground leading-tight mt-0.5">
                        {s.value}
                      </p>
                      <p className="text-[11px] text-tertiary mt-0.5">{s.teamName}</p>
                      <p className="text-[10px] text-muted">{s.context}</p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="bg-surface border border-border rounded-2xl overflow-hidden">
              ...
              <div className="grid grid-cols-3 divide-x divide-border">
                <div className="px-5 py-4 text-center">
                  <p className="text-[22px] font-black text-positive-bright">{best.pf.toLocaleString()}</p>
                  <p className="text-[11px] text-tertiary mt-0.5">Best Season PF</p>
                  <p className="text-[10px] text-muted">{best.seasonLabel}</p>
                </div>
                <div className="px-5 py-4 text-center">
                  <p className="text-[22px] font-black text-info-bright">{Math.round(avgPF).toLocaleString()}</p>
                  <p className="text-[11px] text-tertiary mt-0.5">Avg Season PF</p>
                </div>
                <div className="px-5 py-4 text-center">
                  <p className="text-[22px] font-black text-negative-bright">{worst.pf.toLocaleString()}</p>
                  <p className="text-[11px] text-tertiary mt-0.5">Worst Season PF</p>
                  <p className="text-[10px] text-muted">{worst.seasonLabel}</p>
                </div>
              </div>
            </div>
          );
        ...
        {favouritePlayers.length > 0 && (
          <div className="bg-surface border border-border rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-border flex items-center gap-2">
              ...
              <span className="text-[11px] text-border-strong ml-auto">Most-drafted players in league history</span>
            </div>
            ...
            <div className="divide-y divide-border">
              {favouritePlayers.map((p, i) => (
                <div key={p.playerName} className="flex items-center gap-4 px-5 py-3.5">
                  {/* Rank */}
                  <span className="text-[13px] font-black text-border-strong w-4 shrink-0 tabular-nums">
                    {i + 1}
                  </span>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-[14px] font-bold text-foreground">{p.playerName}</p>
                      <span className="text-[10px] font-bold bg-info-surface text-info border border-info-bright rounded px-1.5 py-0.5">
                        {p.position}
                      </span>
                      {p.proTeam !== '—' && (
                        <span className="text-[10px] font-semibold text-tertiary bg-background border border-border rounded px-1.5 py-0.5">
                          {p.proTeam}
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-1 mt-1 flex-wrap">
                      {p.years.sort().map(y => (
                        <span key={y} className="text-[10px] text-muted bg-surface-secondary rounded px-1.5 py-0.5">
                  ...
                  <div className="shrink-0 text-right">
                    <p className="text-[20px] font-black text-accent leading-none">{p.draftCount}×</p>
                    <p className="text-[10px] text-muted mt-0.5">drafted</p>
                  </div>
                </div>
              ))}
            </div>
