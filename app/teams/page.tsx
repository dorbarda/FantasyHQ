  ...
  if (!hasEspnCredentials()) {
    return (
      <div className="min-h-screen bg-[#F8FAFC] px-4 sm:px-6 lg:px-8 py-6">
        <h1 className="text-[28px] font-black tracking-tight text-[#0F172A] mb-2">Teams</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        {teams.map(team => (
          <Link
            ...
            className="group bg-white border border-[#E2E8F0] rounded-2xl p-5 flex flex-col items-center text-center hover:border-[#C8956C]/60 hover:shadow-md transition-all duration-200"
          >
            {/* Logo */}
            <div className="w-20 h-20 rounded-full overflow-hidden bg-[#F1F5F9] ring-2 ring-[#E2E8F0] group-hover:ring-[#C8956C]/40 transition-all mb-3 shrink-0">
              {team.logo ? (
                <Image
                  src={team.logo}
                ...
                <div className="w-full h-full flex items-center justify-center text-2xl font-black text-[#94A3B8]">
            ...
            {/* Owner name */}
            <p className="text-[14px] font-bold text-[#0F172A] leading-tight mb-0.5">
              {team.ownerName}
            </p>
            {/* Latest team name */}
            <p className="text-[11px] text-[#64748B] truncate w-full mb-3">
              {team.latestTeamName}
            </p>

            {/* Quick stats row */}
            <div className="w-full grid grid-cols-3 gap-1 border-t border-[#F1F5F9] pt-3">
              <div className="flex flex-col items-center">
                <span className="text-[15px] font-black text-[#C8956C]">
                  {team.championships > 0 ? '🏆'.repeat(Math.min(team.championships, 3)) : '—'}
                </span>
                <span className="text-[9px] text-[#94A3B8] uppercase tracking-wide mt-0.5">Titles</span>
              </div>
              <div className="flex flex-col items-center">
                <span className="text-[14px] font-black text-[#0F172A]">{team.winPct}%</span>
                <span className="text-[9px] text-[#94A3B8] uppercase tracking-wide mt-0.5">Win%</span>
              </div>
              <div className="flex flex-col items-center">
                <span className="text-[14px] font-black text-[#0F172A]">{team.seasonsPlayed}</span>
                <span className="text-[9px] text-[#94A3B8] uppercase tracking-wide mt-0.5">Seasons</span>
              </div>
            </div>
          </Link>
        ))}
