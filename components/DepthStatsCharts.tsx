      ...
      <button
        ...
        className={`px-2.5 py-1 rounded-full text-[10px] font-semibold border transition-all ${
          allSelected
            ...
            : 'bg-surface text-tertiary border-border hover:border-muted'
        }`}
      >
        All
      </button>
...
function ScorePPLineChart({
  ...
  return (
    <div className="bg-surface border border-border rounded-xl p-5">
      <h3 className="text-[13px] font-bold text-foreground mb-1">Score per Player — by Week</h3>
      <p className="text-[11px] text-muted mb-3">Team Score ÷ Players Played each week</p>
      <ResponsiveContainer width="100%" height={260}>
        <LineChart data={data} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#F1F5F9" />
          <XAxis dataKey="week" tick={{ fontSize: 10, fill: 'var(--foreground-muted)' }} />
          <YAxis tick={{ fontSize: 10, fill: 'var(--foreground-muted)' }} width={36} />
          <Tooltip
...
function PlayersLineChart({
  ...
  return (
    <div className="bg-surface border border-border rounded-xl p-5">
      <h3 className="text-[13px] font-bold text-foreground mb-1">Players Played — by Week</h3>
      <p className="text-[11px] text-muted mb-3">Total starter games used each week</p>
      <ResponsiveContainer width="100%" height={260}>
        <LineChart data={data} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#F1F5F9" />
          <XAxis dataKey="week" tick={{ fontSize: 10, fill: 'var(--foreground-muted)' }} />
...
function TeamSummaryChart({ summaries }: { summaries: TeamSummary[] }) {
  ...
  return (
    <div className="bg-surface border border-border rounded-xl p-5">
      <h3 className="text-[13px] font-bold text-foreground mb-1">Season Ranking</h3>
      <p className="text-[11px] text-muted mb-4">Avg Score/PP · Win % — sorted by Score/PP</p>
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={data} layout="vertical" margin={{ top: 4, right: 48, left: 8, bottom: 0 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#F1F5F9" horizontal={false} />
          <XAxis type="number" tick={{ fontSize: 10, fill: 'var(--foreground-muted)' }} />
          <YAxis type="category" dataKey="owner" tick={{ fontSize: 10, fill: 'var(--foreground-secondary)' }} width={70} />
          <Tooltip contentStyle={{ fontSize: 11, borderRadius: 8, border: '1px solid var(--border)' }} />
          <Legend wrapperStyle={{ fontSize: 10 }} />
...
function EfficiencyWinRateScatter({ summaries, colorMap }: { summaries: TeamSummary[]; colorMap: Record<string, string> }) {
  ...
  return (
    <div className="bg-surface border border-border rounded-xl p-5">
      <h3 className="text-[13px] font-bold text-foreground mb-1">Efficiency vs Win Rate</h3>
      <p className="text-[11px] text-muted mb-4">Avg efficiency ratio (Score ÷ Players²) vs season win %</p>
      <ResponsiveContainer width="100%" height={280}>
        <ScatterChart margin={{ top: 16, right: 24, left: 0, bottom: 24 }}>
          ...
          <XAxis
            ...
            label={{ value: 'Avg Efficiency', position: 'insideBottom', offset: -12, fontSize: 10, fill: 'var(--foreground-muted)' }}
            tickFormatter={v => Number(v).toFixed(3)}
          />
          <YAxis
            type="number" dataKey="y" name="Win %"
            tick={{ fontSize: 10, fill: 'var(--foreground-muted)' }} width={40}
            tickFormatter={v => `${v}%`}
            domain={[0, 100]}
          />
          ...
          <Tooltip
            ...
            content={({ payload }) => {
              ...
              return (
                <div className="bg-surface border border-border rounded-lg px-3 py-2 text-[11px] shadow-sm">
                  <p className="font-semibold text-foreground">{d.owner}</p>
                  <p className="text-secondary">Efficiency: {Number(d.x).toFixed(3)}</p>
                  <p className="text-secondary">Win rate: {d.y}%</p>
                </div>
              );
            }}
          />
          <Scatter data={data}>
            <LabelList dataKey="owner" position="top" style={{ fontSize: 9, fill: 'var(--foreground-secondary)' }} />
            {data.map((d, i) => (
...
function PlayersScorePPScatter({ rows }: { rows: MatchupDepthRow[] }) {
  ...
  return (
    <div className="bg-surface border border-border rounded-xl p-5">
      <h3 className="text-[13px] font-bold text-foreground mb-1">Players vs Score/PP</h3>
      <p className="text-[11px] text-muted mb-4">Each dot = one team-week · green = win · red = loss</p>
      <ResponsiveContainer width="100%" height={280}>
        <ScatterChart margin={{ top: 4, right: 16, left: 0, bottom: 24 }}>
          ...
          <XAxis
            ...
            domain={[20, 'auto']}
            tick={{ fontSize: 10, fill: 'var(--foreground-muted)' }}
            label={{ value: 'Players Played', position: 'insideBottom', offset: -12, fontSize: 10, fill: 'var(--foreground-muted)' }}
          />
          <YAxis
          ...
          <Tooltip
            ...
            content={({ payload }) => {
              ...
              return (
                <div className="bg-surface border border-border rounded-lg px-3 py-2 text-[11px] shadow-sm">
                  <p className="font-semibold text-foreground">{d.owner} — Wk {d.week}</p>
                  <p className="text-secondary">{d.x} players · {Number(d.y).toFixed(1)} score/pp</p>
                  {d.won !== null && (
                    <p className={d.won ? 'text-positive' : 'text-negative'}>{d.won ? 'Win' : 'Loss'}</p>
                  )}
                </div>
              );
            }}
  ...
  return (
    <div className="space-y-5">
      <div className="bg-surface border border-border rounded-xl p-5 animate-pulse">
        <div className="h-3 w-32 bg-surface-secondary rounded mb-3" />
        <div className="flex gap-2 mb-4">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-6 w-14 bg-surface-secondary rounded-full" />
          ))}
        </div>
        <div className="h-2 w-40 bg-surface-secondary rounded" />
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="bg-surface border border-border rounded-xl p-5 animate-pulse">
            <div className="h-3 w-40 bg-surface-secondary rounded mb-2" />
            <div className="h-2 w-56 bg-surface-secondary rounded mb-4" />
            <div className="h-64 bg-background rounded-lg" />
          </div>
        ))}
        <div className="lg:col-span-2 bg-surface border border-border rounded-xl p-5 animate-pulse">
          <div className="h-3 w-40 bg-surface-secondary rounded mb-2" />
          <div className="h-2 w-56 bg-surface-secondary rounded mb-4" />
          <div className="h-72 bg-background rounded-lg" />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      <div className="bg-surface border border-border rounded-xl px-5 py-4">
        <p className="text-[11px] font-semibold uppercase tracking-widest text-muted mb-3">Filter teams</p>
        <TeamSelector
          owners={owners}
          selected={selected}
          colorMap={colorMap}
        ...
        />
        <p className="text-[10px] text-border-strong">Applies to the two charts below</p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
  ...
  if (!depthData || depthData.rows.length === 0) {
    return (
      <div className="border border-border rounded-lg px-6 py-10 text-center bg-surface">
        ...
        <p className="text-[13px] text-muted mt-1">Stats will appear once matchups are completed.</p>
      </div>
    );
  }
