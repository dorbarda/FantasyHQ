    ...
    <div className="space-y-3">
      {players.map((p, i) => (
        <div key={p.playerId} className="flex items-center gap-3">
          <span className="text-[12px] font-semibold text-[#475569] w-4 text-right shrink-0">
            {i + 1}
          </span>
          ...
          <span className="text-[13px] font-bold tabular-nums text-[#0F172A] w-8 text-right shrink-0">
            {p.addCount}
          </span>
        </div>
      ))}
      ...
      {teams.map((team, rank) => (
        ...
        >
          ...
          <div className="flex items-start justify-between gap-2 mb-3">
            <div className="min-w-0">
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] font-semibold text-[#475569]">#{rank + 1}</span>
            ...
            <div className="text-right shrink-0">
              <p className="text-[22px] font-bold text-[#0F172A] leading-none tabular-nums">{team.totalAdds}</p>
              <p className="text-[10px] text-[#475569] uppercase tracking-wide">total adds</p>
            </div>
          </div>

          {/* Activity bar */}
          <div className="mb-3">
            <div className="bg-[#F1F5F9] rounded-full h-1 overflow-hidden">
              <div
          ...
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-[#475569] shrink-0">Top target</span>
            <div className="flex items-center gap-1.5 min-w-0">
              <TeamBadge team={team.topPlayerProTeam} />
              <span className="text-[12px] font-semibold text-[#0F172A] truncate">{team.topPlayer}</span>
            </div>
            <span className="text-[11px] font-semibold text-[#475569] shrink-0 ml-auto">
  ...
  return (
    <div className="border border-[#E2E8F0] rounded-lg bg-white p-4">
      <div className="flex items-center justify-between mb-3">
        <span className="text-[11px] text-[#475569] font-medium">{date}</span>
      </div>
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] gap-2 items-start">
        ...
        <div>
          ...
          <p className="text-[10px] text-[#475569] uppercase tracking-wide mb-1">Received</p>
          <div className="space-y-1">
            {trade.teamAReceived.map(p => (
              <div key={p.playerId} className="flex items-center gap-1.5">
                <TeamBadge team={p.proTeam} />
                <span className="text-[11px] text-[#0F172A] font-medium truncate">{p.playerName}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-4 flex-wrap">
        <div className="border border-[#E2E8F0] rounded-lg px-5 py-3 bg-white">
          <p className="text-[24px] font-bold text-[#0F172A] leading-none tabular-nums">{data.totalAdds}</p>
          <p className="text-[11px] text-[#94A3B8] uppercase tracking-wide mt-0.5">Total adds this season</p>
        </div>
        <div className="border border-[#E2E8F0] rounded-lg px-5 py-3 bg-white">
          <p className="text-[24px] font-bold text-[#0F172A] leading-none">{data.topAddedPlayers[0]?.playerName?.split(' ').pop() ?? '—'}</p>
          <p className="text-[11px] text-[#94A3B8] uppercase tracking-wide mt-0.5">Most added player</p>
        </div>
        <div className="border border-[#E2E8F0] rounded-lg px-5 py-3 bg-white">
          <p className="text-[24px] font-bold text-[#0F172A] leading-none">{data.addsByNbaTeam[0]?.team ?? '—'}</p>
